...
void ADC_voidInit(void)
{
	/*	1- Select Vref = AVCC
		...
		5- Enable ADC
	 */
	 //1-selecet the voltage refarance to work ADC 
    // to done at vcc refrance clear bit 7 and set bit 6 at register admux
	CLR_BIT(ADC_u8_ADMUX_REG,7);
	SET_BIT(ADC_u8_ADMUX_REG,6);
    //2-ADC Left Adjust Result===>>> CLR BIT 5
	CLR_BIT(ADC_u8_ADMUX_REG,5);
    // Disable the out trigger 
	CLR_BIT(ADC_u8_ADCSRA_REG,5);
	//Select the ADC Prescaler Select Bits 0 ,1 ,2==>>>>>>clk 64 
	// 64==>> 110 
